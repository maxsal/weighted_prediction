# takes a numeric range of phecodes as a character and translates it into
# all numeric phecode possibilities
# author: lars fritsche
# source: https://raw.githubusercontent.com/umich-cphds/createUKBphenome/master/scripts/function.expandPhecodes.r
...
expandPhecodes <- function(x,addIntegers=T){
  ...
  if (grepl("\\-",x)) {
    ...
    seq1 <- formatC(seq1, format = 'f',
                    digits = ndigits,
                    width = nprefix + ndigits + addDot, flag = 0)
    # add integers if within range 
    if (addIntegers) seq1 <- unique(sort(c(
      seq1, gsub("\\..+", "",
                 seq1[which(round(as.numeric(seq1)) == as.numeric(seq1))]))))
    
    if (ndigits == 2) {
      seq2 <- seq(as.numeric(i1[1]), as.numeric(i1[2]), (1/10^(ndigits - 1)))
      seq2 <- formatC(seq2, format = 'f',
                      digits = ndigits - 1,
                      width = nprefix + ndigits + addDot - 1, flag = 0)
